// kernel/agent_loader.c — ultra-verbose PIE loader with RELATIVE relocs + arena fallback
// Verbosity: set LOADER_VERBOSE to 0/1. 1 = very chatty.
...
static void hexdump_window(const uint8_t* p, size_t before, size_t after){
	if(!p) return;
	const uint8_t* start = p - (before < (size_t)(uintptr_t)p ? before : before);
	size_t total = before + after;
	for(size_t i=0;i<total;i+=16){
		size_t n = (total - i > 16) ? 16 : (total - i);
		kprintf("[dump] %016llx : ", (unsigned long long)(uintptr_t)(start + i));
		for(size_t j=0;j<16;j++){
			if(j<n) kprintf("%02x ", start[i+j]);
			else    kprintf("   ");
		}
		kprintf(" |");
...
static int json_get_int(const char* json, const char* key){
	if(!json||!key) return -1; char pat[64]; snprintf(pat,sizeof(pat),"\"%s\"",key);
	const char* p=strstr(json,pat); if(!p) return -1;
	p += strlen(pat); p = skip_ws(p); if(*p!=':') return -1; p++; p=skip_ws(p);
...
/* -------------------- Arena (OOM fallback) ------------------- */
#define AGENT_ARENA_SIZE (1<<20) /* 1 MiB total for agent images */
                                     ...
                                     const Elf64_Ehdr* eh, const void* img, size_t sz)
{
	...
	size_t applied = 0;
	auto do_table = [&](Elf64_Addr tab, Elf64_Addr tsz){
		if(!tab || !tsz) return;
		if (tab < lo) return;
		uint64_t off = tab - lo;
		if (off + tsz > (uint64_t)SIZE_MAX) return;
...
static int load_agent_elf_impl(const void* img, size_t sz, const char* path, int prio){
	...
	for(uint16_t i=0;i<eh->e_phnum;i++){
		...
		LOGV("[loader] PT_LOAD[%u]: vaddr=0x%llx filesz=%llu memsz=%llu flags=0x%x -> [%p..%p)\n",
		     (unsigned)i,
	...
	/* Entry byte inspection + window dump */
	if (LOADER_VERBOSE){
		...
		size_t before = 32, after = 32;
		LOGV("[loader] dumping %zuB around entry %p\n", before+after, (void*)entry_addr);
		hexdump_window(entry_ptr, before, after);
	}
...
/* --------------------------- Public API ---------------------- */
typedef enum {
	AGENT_FORMAT_ELF, AGENT_FORMAT_MACHO2, AGENT_FORMAT_NOSM, AGENT_FORMAT_FLAT, AGENT_FORMAT_UNKNOWN
} agent_format_t;
