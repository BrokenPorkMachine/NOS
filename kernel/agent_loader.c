...
#include "agent.h"
#include "../Task/thread.h"            // thread_create_with_priority
#include "../../user/libc/libc.h"      // printf/snprintf/memcmp/memcpy/str* etc.
#include <stdint.h>

// Use kernel console print (provide one of these)
extern int kprintf(const char *fmt, ...);
// If your console function is named printf in kernel/printf.c, uncomment next line:
// #define kprintf printf

// ---- File read hook ---------------------------------------------------------
// Provide a function that reads a whole file into memory (kmalloc'ed).
// The function should set *out to a kmalloc'ed buffer and *out_sz to its size.
// Return 0 on success, <0 on error.
typedef int (*agent_read_file_fn)(const char *path, void **out, size_t *out_sz);

// Optional free hook (use your kheap if available). If NULL, buffer is leaked (not ideal).
...
// ---- Entry registry (symbol resolution by name) -----------------------------
...
// ---- Register + spawn -------------------------------------------------------
static int register_and_spawn_from_manifest(const char *json, int prio) {
    ...
    if (!fn) {
        kprintf("[loader] entry symbol \"%s\" not found for \"%s\"\n", entry, m.name);
        return -1;
    }
    ...
    // Spawn as its own kernel thread (don’t run inline)
    thread_t *t = thread_create_with_priority((void(*)(void))fn, prio > 0 ? prio : 200);
    if (!t) {
        kprintf("[loader] failed to create thread for \"%s\"\n", m.name);
        return -1;
    }
    return 0;
}

// ---- Format-specific loaders (manifested formats) ---------------------------
static int load_agent_macho2(const void *image, size_t size, int prio) {
    char manifest[512];
    if (extract_manifest_macho2(image, size, manifest, sizeof(manifest)) == 0)
        return register_and_spawn_from_manifest(manifest, prio);
    return -1;
}
...
static int load_agent_macho(const void *image, size_t size, int prio) {
    (void)image; (void)size; (void)prio;
    kprintf("[loader] Mach-O loader not fully implemented\n");
    return -1;
}
static int load_agent_flat(const void *image, size_t size, int prio) {
    (void)image; (void)size; (void)prio;
    kprintf("[loader] flat loader stub (no manifest)\n");
    return -1;
}
static int load_agent_nosm(const void *image, size_t size, int prio) {
    (void)image; (void)size; (void)prio;
    kprintf("[loader] NOSM loader stub (no manifest)\n");
    return -1;
}

// ---- Public loader API (by buffer / auto-detect) ----------------------------
int load_agent_auto_with_prio(const void *image, size_t size, int prio) {
    switch (detect_agent_format(image, size)) {
        case AGENT_FORMAT_MACHO2: return load_agent_macho2(image, size, prio);
        case AGENT_FORMAT_MACHO:  return load_agent_macho(image, size, prio);
        case AGENT_FORMAT_ELF:    return load_agent_elf(image, size, prio);
        case AGENT_FORMAT_FLAT:   return load_agent_flat(image, size, prio);
        case AGENT_FORMAT_NOSM:   return load_agent_nosm(image, size, prio);
        default:                  return -1;
    }
}
...
// Backwards-compat wrappers (no prio)
...
// ---- Convenience: load + run from a filesystem path -------------------------
int agent_loader_run_from_path(const char *path, int prio) {
    if (!g_read_file) {
        kprintf("[loader] no file reader hook installed; cannot load \"%s\"\n", path ? path : "(null)");
        return -1;
    }
    void *buf = NULL; size_t sz = 0;
    int rc = g_read_file(path, &buf, &sz);
    if (rc < 0) {
        kprintf("[loader] read failed for \"%s\"\n", path);
        return rc;
    }
    rc = load_agent_auto_with_prio(buf, sz, prio);
