...
// Reader/free hooks provided by kernel (set during boot)
...
void agent_loader_set_read(agent_read_file_fn reader, agent_free_fn freer){
    g_read_file = reader; g_free_fn = freer;
}
...
static const void *memmem_local(const void *hay, size_t haylen, const void *need, size_t needlen){
    ...
    if (needlen==0) return hay;
    for (size_t i=0;i+needlen<=haylen;++i) if (memcmp(h+i,n,needlen)==0) return h+i;
    return NULL;
}
...
static int load_agent_macho(const void *image,size_t size,int prio){ (void)image;(void)size;(void)prio; kprintf("[loader] Mach-O not implemented\n"); return -1; }
static int load_agent_flat (const void *image,size_t size,int prio){ (void)image;(void)size;(void)prio; kprintf("[loader] flat loader stub\n"); return -1; }
static int load_agent_nosm (const void *image,size_t size,int prio){ (void)image;(void)size;(void)prio; kprintf("[loader] NOSM loader stub\n"); return -1; }
