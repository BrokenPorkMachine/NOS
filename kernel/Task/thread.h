...
typedef struct thread {
    ...
    int            priority;  // NEW: priority (0 = lowest, 255 = highest)
...
// Create a new kernel thread with default priority (mid-level)
thread_t *thread_create(void (*func)(void));

// Create a new kernel thread with an entrypoint and an explicit priority
...
// Mark thread as ready (for scheduler).  If its priority
